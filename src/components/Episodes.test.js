...
//I created a fake DUMMY variable and JSON formatted and copied actual data
//from the real API
...
//Basic first test
test ('Basic test to assure that Episodes.js RENDERS', () => {
    //if we get an error of map of undefined it's because we need to include props that are being passed down
    //here we are passing a fake props as an empty array
    render(<Episodes episodes={[]}/>);
})

test("Render list of episodes after API CALL", () => {
    //1.render component with empty array for the episodes props before state change
    const { rerender, queryAllByTestId, debug } = render(<Episodes episodes={[]}/>);
    
    //dubug shows the state before and after similar to console.log
    // debug();

    

    //2.re-render the component with episodes and where the stata change happens     
    ...
    expect(episodesData).toHaveLength(1) 
